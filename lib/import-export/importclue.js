...
// Basic template to be filled with data for each day
const blank = {
  date: null,
  bleeding: {
    exclude: null,
...
/**
 * Authenticate with Clue API given user email & password
 * @param email {string} user email
 * @param password {string} user password
 * @returns {string} accessToken to use with API
 */
...
/**
 * Fetch all available data from the API given the access token.
 * @param accessToken {string} API access token
 * @returns {JSON} symptom data
 * @throws {NetworkError: Error} if internet connection could not be established or timed out
 * @throws {ParsingError: Error} if the api result has an unexpected format
 */
const fetchData = async (accessToken) => {
  ...
  try {
    ...
    if (!response.ok) {
      // this is just for local debugging, is immediately caught again
...
/**
 * Reorders the data to being sorted by date. To be called immediately after authenticate.
 * @param data {JSON} - data as obtained by Clue export or API call. Format explained in FAQ
 * @returns {{JSON}} -   "YYYY-MM-DD": [
 *     symptom1,
 *     symptom2
 *   ], YYYY-MM-DD: ...
 */
...
/**
 * Responsible for the logic of converting Clue data to drip data.
 * Adjust this if either app adds new data points.
 * @param groupedData {JSON} output of combineIntoDate
 * @returns {JSON} array of "blanks"
 */
const parseToDrip = async (groupedData) => {
  const data = []
  for (const dateKey in groupedData) {
    const blankCopy = JSON.parse(JSON.stringify(blank)) //copy template
    blankCopy.date = dateKey
    for (const symptomKey in groupedData[dateKey]) {
      const symptom = groupedData[dateKey][symptomKey]
      switch (symptom.type) {
        case 'sex_life':
          symptom.value.forEach((v) => {
            switch (v.option) {
              case 'masturbation':
          ...
          symptom.value.forEach((v) => {
            switch (v.option) {
              ...
              case 'egg_white':
                ...
                break
              default:
                blankCopy.note.value += `Discharge: ${v.option} `
                break
            }
          })
          break
...
/**
 * Maps each template to a row in cycledays
 * Adjust columnNames when new symptom is added
 * @param cycleDays {JSON} array of drip data points in "blank" format.
 * @returns {string} csv string of data points in drip format
 */

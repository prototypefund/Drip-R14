...
function generateOverview(t) {
  ...
  const lengthStats = getCycleLengthStats(cycleLengths)
  const bleedingStats = getBleedingStats(statsData)
...
<div class="statistics">\
  ...
  } <span class="highlight">${lengthStats.mean}</span></div>\
  <div class="stat-content">${
    t('export.overview.statistics.bleedingLength', { count: numbers }) + ': '
  } <span class="highlight">${bleedingStats}</span></div>\
</div>
